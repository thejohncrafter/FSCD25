...
private
  lemma-con
    ...
    → (T : Ty B) → (x : Tm B)
    → (xt : XT (B , T) Δ)
    → xt-wkn-con (xt-sub xt (sub x)) σ
    ≡ xt-sub-con (xt-wkn xt (σ ↑ T)) (sub (x [ σ ]))
  ...
  lemma-ty
    ...
    → (T : Ty B) → (x : Tm B)
    → (xt : XT (B , T) Δ)
    →   A [ xt-sub-op xt (sub x) ] [ xt-wkn-op (xt-sub xt (sub x)) σ ]
    ≡* A [ xt-wkn-op xt (σ ↑ T) ] [ xt-sub-op (xt-wkn xt (σ ↑ T)) (sub (x [ σ ])) ]
  ...
  lemma-var
    ...
    → (T : Ty B) → (x : Tm B)
    → (xt : XT (B , T) Δ)
    →   v [ xt-sub-op xt (sub x) ] [ xt-wkn-op (xt-sub xt (sub x)) σ ]
    ≡* v [ xt-wkn-op xt (σ ↑ T) ] [ xt-sub-op (xt-wkn xt (σ ↑ T)) (sub (x [ σ ])) ]
  ...
  lemma-tm
    ...
    → (T : Ty B) → (x : Tm B)
    → (xt : XT (B , T) Δ)
    →   t [ xt-sub-op xt (sub x) ] [ xt-wkn-op (xt-sub xt (sub x)) σ ]
    ≡* t [ xt-wkn-op xt (σ ↑ T) ] [ xt-sub-op (xt-wkn xt (σ ↑ T)) (sub (x [ σ ])) ]
  ...
  lemma-var (there v) σ T x zero       = refl
  lemma-var (there {_} {S} v) σ T x (succ xt _) =
    begin
        v [ xt-sub-op xt (sub x) ] [ wkn (S [ xt-sub-op xt (sub x) ]) ] [ xt-wkn-op (xt-sub xt (sub x)) σ ↑ (S [ xt-sub-op xt (sub x) ]) ]
      ≡*⟨ ≡*-of-eq (up-wkn-tm _ _ _) ⟩
        v [ xt-sub-op xt (sub x) ] [ xt-wkn-op (xt-sub xt (sub x)) σ ] [ wkn _ ]
      ≡*⟨ map-heq-wkn-tm (lemma-var v σ _ _ _ ) (lemma-ty _ σ _ _ _) ⟩
        v [ xt-wkn-op xt (σ ↑ T) ] [ xt-sub-op (xt-wkn xt (σ ↑ T)) (sub (x [ σ ])) ] [ wkn _ ]
    ∎

  lemma-tm (var v)       σ T x xt = lemma-var _ σ _ _ _
...
sub-wkn-ty T A σ x = eq-of-ty-idxeq (lemma-ty A σ T x zero)
...
sub-wkn-var T v σ x = eq-of-tm-idxeq (lemma-var v σ T x zero)
...
sub-wkn-tm T t σ x = eq-of-tm-idxeq (lemma-tm t σ T x zero)
